...
@interface resumeGameViewController () <UIActionSheetDelegate>
...
@implementation resumeGameViewController
...
- (NSUInteger)numberOfItemsInCarousel:(iCarousel *)carousel
{
    ...
    NSArray *mazeso = [Maze getMazeByResumedGames:self.document.managedObjectContext];
...
{    
    if (!self.mazes) {
        self.mazes = [Maze getMazeByResumedGames:self.document.managedObjectContext];
    }
    ...
    if(index == 0){
        ...
        UIImageView *stripeImageview =[UikitFramework createImageViewWithImage:[UikitFramework getStripeByDifficulty:maze.dificulty.name] positionX:179.5 - stripe.size.width - 4  positionY:5];
        
        [button addSubview:stripeImageview];
...
-(void)viewDidAppear:(BOOL)animated
{
    ...
    UILabel *label = [UikitFramework createLableWithText:@"SELECT IMAGE" positionX:0 positionY:0 width:self.view.frame.size.width height:100];
...
- (void)buttonTapped:(UIButton *)sender
{
    if (sender.tag == 0) {
        ...
        [self performSegueWithIdentifier:@"resumed detail view" sender:self];
    }
}
